...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: `Best ${category.name} | LocalFind`,
    description: `Discover the top-rated ${category.name} in your area on LocalFind.`,
  };
}
    ...
    <div className="bg-gray-50 min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Best {category.name}</h1>
          <p className="text-gray-600 text-lg">
            {category.description || `Explore top-rated ${category.name.toLowerCase()} around you.`}
          </p>
        </div>
        ...
        {businesses.length === 0 ? (
          <div className="bg-white p-10 rounded-xl border border-gray-200 text-center shadow-sm">
            ...
            <p className="text-gray-500 mb-6">We couldn't find any {category.name.toLowerCase()} listings right now.</p>
              ...
              return (
                <div key={business.id} className="bg-white rounded-xl overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center text-gray-600 text-sm mb-5 mt-3">
